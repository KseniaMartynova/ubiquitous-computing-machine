...
int main(int argc, char* argv[]) {
    ...
    // Обращение матрицы с помощью SVD и замер времени выполнения
    auto start = std::chrono::high_resolution_clock::now();
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(A, Eigen::ComputeThinU | Eigen::ComputeThinV);
    Eigen::MatrixXd A_inv = svd.solve(Eigen::MatrixXd::Identity(N, N));
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration = end - start;

    // Вывод времени выполнения
    std::cout << "Time to invert matrix using SVD: " << duration.count() << " seconds" << std::endl;
    ...
    if (error < 1e-10) {
        std::cout << "Matrix inversion using SVD is correct." << std::endl;
    } else {
        std::cout << "Matrix inversion using SVD is incorrect." << std::endl;
    }
