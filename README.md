...
Директория [MKL](runks/build/mkl) содержит в себе папки для четырех операций, каждай папка содержит в себе следующее: [докер файл для нужного контейнера](runks/build/mkl/cholesky/Dockerfile.mklcho), [скрипт для сборки контейнера](runks/build/mkl/cholesky/build.sh),  [скрипт для запуска рассчетов ](runks/build/mkl/cholesky/run.sh), [сама программа для выполнения алгебраической операции](runks/build/mkl/cholesky/mklCho.sh), и папка с [результатами](runks/build/mkl/cholesky/results)
## Начало работы
1. Для начала клонируем репозиторий: 
```
git clone https://github.com/KseniaMartynova/ubiquitous-computing-machine
```
2. Каждый подразедел [директории build](runks/build) является папкой с названием библиотеки, которая содержит в себе директории, соответствующие алгебратическим операциям (рассмотрим на примере [MKL](runks/build/mkl)). Директория [MKL](runks/build/mkl) содержит в себе папки для четрыех операций, каждай папка содержит в себе следующее: [докер файл для нужного контейнера](runks/build/mkl/cholesky/Dockerfile.mklcho), [скрипт для сборки контейнера](runks/build/mkl/cholesky/build.sh)   build.sh  в себе возможность запустить сборку всех контейнеров одновременно, подробнее про сборку будет ниже в подразделах. 
...
Итак, возьмем для примера обращение c помощью разложения Холецкого. Попробуем сделать докер файл для программы [mklCho.cpp](runks/build/mkl/mklCho.cpp). 
...
Ниже представлены черновые наработки для Eigen, Armadillo, CUDA cuBLAS и cuSOLVER. При желании и должном упорстве читающий может довести до ума наши наработки.  
