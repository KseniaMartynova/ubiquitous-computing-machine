...
```
2. Далее я создала dockerfile, в котором содержится все, что мне нужно для дальнейших вычилений, например, сама программа, которая производит обращение положительно определенной матрицы и замеряет время ее обращения.
```
...
./mkl
```
Вместо третьего пункта на самом деле проще не заходить в контейнер и запустить следующее с нужным размером матрицы:
```
