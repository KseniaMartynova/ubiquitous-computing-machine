...
```
2. Каждый подразедел [директории](runks/build) является папкой с названием библиотеки, которая содержит в себе директории, соответствующие алгебратическим операциям (рассмотрим на примере [MKL](runks/build/mkl)).  build.sh  в себе возможность запустить сборку всех контейнеров одновременно, подробнее про сборку будет ниже в подразделах. 
