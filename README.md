...
```
2. В каждом подразеделе  [build.sh](runks/build.sh) содержит в себе возможность запустить сборку всех контейнеров одновременно, подробнее про сборку будет ниже в подразделах. 
