...
- [1. MKL](#Intel_Math_Kernel_Library (MKL))
- [2. OpenBLAS - LAPACK](#OpenBLAS_-_LAPACK)
- [3. NumPy](#NumPy)
...
## Введение
Этот репозиторий это практическая часть статьи: "Сравнительный анализ производительности операций линейной алгебры в контейнеризованных средах: MKL, OpenBLAS/LAPACK и NumPy". Он создан, и стал открытым и общедоступным для того, чтобы любой желающий и облающий минимальными компетенциями в работе с Linux системами, мог воспроизвести все наши вычисления и сделать собственные выводы.
...
Ниже будет краткая инструкция и представлены комманды для базового запуска, их можно модифицировать под конкретную операцию.

### Перед началом работы стоит уточнить структуку репозитория. 
Каждый подразедел [директории build](runks/build) является папкой с названием библиотеки, которая содержит в себе директории, соответствующие алгебратическим операциям (рассмотрим на примере [MKL](runks/build/mkl)). 

Директория [MKL](runks/build/mkl) содержит в себе папки для четырех операций, каждай папка содержит в себе следующее: [докер файл для нужного контейнера](runks/build/mkl/cholesky/Dockerfile.mklcho), [скрипт для сборки контейнера](runks/build/mkl/cholesky/build.sh),  [скрипт для запуска рассчетов ](runks/build/mkl/cholesky/run.sh), [сама программа для выполнения алгебраической операции](runks/build/mkl/cholesky/mklCho.sh), и папка с [результатами](runks/build/mkl/cholesky/results). Директории с библиотеками отделены друг от друга, потому что я считаю, что так удобнее и Вы можете запускать только интересующие вас контейнеры с библиотеками и алгебраические операции.
## Начало работы
1. Для начала клонируем репозиторий: 
```
git clone https://github.com/KseniaMartynova/ubiquitous-computing-machine
```
2. Далее нужно выбрать интересующую библиотеку и алгебраическую операцию и запустить build скрипт (подразумевается, что на вашем устройстве уже установлено все необходимое для запуска Docker файлов). Запустится сборка контейнера, которая должна завершиться успешно. 

Как все мы знаем запустить файл bash в командной строке можно так:
```
bash build.sh
```
3. Осталось самое простое, файл run.sh, который запускает измерения времени алгебраической операций. 
```
bash run.sh
```
...
## Продолжение работы
Для самых любопытных я сделала следующий раздел с подробным использованием контейнеров.
...
Итак, возьмем для примера обращение c помощью разложения Холецкого. Попробуем сделать докер файл для программы [mklCho.cpp](runks/build/mkl/cholesky/mklCho.cpp). 

1. Cкачаем образ из официального [dockerhub intel](https://hub.docker.com/r/intel/oneapi-basekit) (я использую версию: intel/oneapi-basekit:2025.0.1-0-devel-ubuntu22.04).
```
docker pull intel/oneapi-basekit:2025.0.1-0-devel-ubuntu22.04
```
2. Далее соберем dockerfile, в котором содержится все, что нам нужно для дальнейших вычислений, конечно же сама программа, которая производит обращение положительно определенной матрицы и замеряет время ее обращения.
```
...
``` 
Использование контейнера в данном случае и в случае OpenBlas-Lapack оправдан: вам не нужно устанавливать никакие зависимости, кроме непосредственно докер образа.
...
## Результаты
Практическим образом было доказано на примере numpy, что контейнеры не вносят значительных искажений в результаты бенчмаркинга. 

Сводную таблицу результатов времени совершения операции с помощью наших библиотек вы можете найти в [результаты](runks/result/analyze_end(1).md)

Подробнее о результатах можно прочитать в ССЫЛКА НА СТАТЬЮ

Ниже представлены черновые наработки для Eigen, Armadillo, CUDA cuBLAS и cuSOLVER, так же существуют черновые наработки для некоторых других алгебраических операций. Все это находится в [draft](draft) При желании и должном упорстве читающий может довести до ума наши наработки (в этом случае просьба связаться с владелицей репозитория).  
