...
def read_times(directory, prefix):
    ...
    Считывает размеры матриц и времена из файлов вида:
        <prefix>_svd_size_<N>.txt
    Возвращает отсортированные массивы (n, time).
    """
    pattern = os.path.join(directory, f"{prefix}_svd_size_*.txt")
...
plt.title('Аппроксимация времени Псевдообращения (svd) (линейная регрессия в log-log)',
          fontsize=14, pad=15)
